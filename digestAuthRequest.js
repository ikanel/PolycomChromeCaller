var digestAuthRequest = function (method, url, username, password) {
	...
	this.makeUnauthenticatedRequest = function(data) {
		...
		if (self.post) {
			self.firstRequest.setRequestHeader('Content-type', 'application/x-com-polycom-spipx');
		}
	...
	this.makeAuthenticatedRequest= function() {
		...
		self.authenticatedRequest.setRequestHeader('Content-type', 'application/x-com-polycom-spipx');
	...
	this.formulateResponse = function() {
		...
		var HA2 = CryptoJS.MD5('POST:/push').toString();
